...
class RotunbotPathCfg(RotunbotVelCleanCfg):
    class env(RotunbotVelCleanCfg.env):
        ...
        # The actor receives 20 frames of 19-D tracking state plus one current
    ...
    class path:
        ...
        include_curvature_observation = os.environ.get("PATH_V5_CURVATURE_OBS", "0") == "1"
        prior_tight_curvature = 0.38
        prior_r2_curvature = 0.45
        prior_s_mid_curvature = 0.38
        prior_s_mid_drive = 0.40
        prior_s_mid_stop_distance = 2.00
        ...
        prior_tight_stop_distance = 2.40
        prior_tight_cross_track_kp = 0.10
        prior_tight_heading_kp = 0.80
        prior_moderate_long_length = 4.60
        prior_moderate_long_drive = 0.35
        prior_moderate_long_stop_distance = 1.20
        prior_moderate_long_cross_track_kp = 0.10
        prior_moderate_long_heading_kp = 0.50
        prior_s_lookahead = 1.20
        prior_s_short_length = 4.50
        prior_s_short_drive = float(os.environ.get("PATH_PRIOR_S_SHORT_DRIVE", "0.25"))
        prior_s_long_drive = float(os.environ.get("PATH_PRIOR_S_LONG_DRIVE", "0.20"))
        prior_s_tight_short_length = 5.50
        prior_s_tight_short_drive = 0.35
        prior_s_tight_long_drive = float(os.environ.get("PATH_PRIOR_S_TIGHT_LONG_DRIVE", "0.45"))
        prior_s_tight_stop_distance = 1.20
        prior_s_tight_short_cross_track_kp = 0.10
        prior_s_tight_long_cross_track_kp = float(os.environ.get("PATH_PRIOR_S_TIGHT_LONG_CROSS", "0.10"))
        prior_s_tight_short_heading_kp = 0.50
        prior_s_tight_long_heading_kp = float(os.environ.get("PATH_PRIOR_S_TIGHT_LONG_HEADING", "0.50"))
        prior_s_complex_boundary_threshold = 0.05
        prior_s_regular_long_cross_track_kp = 0.05
        prior_s_regular_long_heading_kp = 0.00
        prior_s_tight_min_lookahead = 1.00
        prior_s_tight_lookahead_fraction = 0.20
        prior_s_stop_distance = float(os.environ.get("PATH_PRIOR_S_STOP", "1.00"))
        prior_s_cross_track_kp = float(os.environ.get("PATH_PRIOR_S_CROSS", "0.10"))
        prior_s_heading_kp = float(os.environ.get("PATH_PRIOR_S_HEADING", "1.00"))
        prior_speed_kp = 0.65
        prior_gain_offset = 0.092
        prior_gain_slope = 0.92
        prior_gain_min = 0.18
        prior_gain_max = 0.75
        prior_residual_scale_1 = float(os.environ.get("PATH_RESIDUAL_SCALE_1", "0.15"))
        prior_residual_scale_2 = float(os.environ.get("PATH_RESIDUAL_SCALE_2", "0.15"))
        prior_endpoint_pure_pursuit = os.environ.get("PATH_ENDPOINT_PP", "0") == "1"
        prior_endpoint_blend_distance = float(os.environ.get("PATH_ENDPOINT_PP_DISTANCE", "2.0"))
        prior_endpoint_max_curvature = float(os.environ.get("PATH_ENDPOINT_PP_MAX_CURVATURE", "0.50"))
        ...
        # Held-out curvature for the generalisation split.  The stage lists
        # above only ever contain {0.20, 0.25, 0.3333333333, 0.40, 0.50}; none of
        # those values appear below, so PATH_LAYOUT_SPLIT=test measures transfer
        # to unseen curvature instead of recall of the trained values:
        #   0.15 extrapolates below the trained range,
        #   0.30 and 0.45 interpolate between trained values.
        heldout_curvature_values = [0.15, 0.30, 0.45]
        heldout_s_curvature_values = [0.15, 0.30, 0.45]
    ...
    class rewards(RotunbotVelCleanCfg.rewards):
        ...
        class scales:
            ...
            policy_residual = float(os.environ.get("PATH_RESIDUAL_PENALTY", "-0.20")) if os.environ.get("PATH_USE_ACTION_PRIOR", "0") == "1" else 0.0
            path_deviation = -0.25
            action_rate = -0.003
            target_rate = -0.03
