...
def plot_state2(torque_data, vel_data, pos_data, quat_data, ang_vel_data, lin_vel_data, base_pos_data, euler_data, ang_data, yaw_vel):
    """
    绘制关节数据图像，分成两个子图。
    ...
    a = axs[4,1]
    a.plot(time, yaw_vel, label='Yaw_vel', color='green')
    a.set_title('Yaw_Vel vs Time')
    a.set_xlabel('Time')
    a.set_ylabel('Yaw_Vel')
    a.legend()
    a.grid(True)
...
data_record = True
Torque_Output_index = np.zeros(22)
...
obstacle_A_pos = [3.5, 1.5, 0.6]
obstacle_B_pos = [-1.0, -2.0, 0.6]
obstacle_C_pos = [-3.5, 1.5, 0.6]
obstacle_D_pos = [-1.0, -2.0, 0.6]
...
obstacle_A_asset = gym.create_box(sim, *[0.4, 4.0, 1.2], obstacle_opts)
obstacle_B_asset = gym.create_box(sim, *[4.5, 0.4, 1.2], obstacle_opts)

initial_euler = [0.0, -0.11, -0.41]  # 初始欧拉角
...
initial_pose.r = gymapi.Quat( initial_quat[0], initial_quat[1], initial_quat[2], initial_quat[3])

obstacle_A_start_pose = gymapi.Transform()
obstacle_A_start_pose.p = gymapi.Vec3(*obstacle_A_pos)
obstacle_A_start_pose.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)

obstacle_B_start_pose = gymapi.Transform()
obstacle_B_start_pose.p = gymapi.Vec3(*obstacle_B_pos)
obstacle_B_start_pose.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)
...
obstacle_A_actor = gym.create_actor(env0, obstacle_A_asset, obstacle_A_start_pose, 'obstacle', 0, 1, 0)
obstacle_B_actor = gym.create_actor(env0, obstacle_B_asset, obstacle_B_start_pose, 'obstacle', 0, 1, 0)

# Set colors
gym.set_rigid_body_color(env0, obstacle_A_actor, 0, gymapi.MESH_VISUAL, obstacle_A_color)
gym.set_rigid_body_color(env0, obstacle_B_actor, 0, gymapi.MESH_VISUAL, obstacle_B_color)
gym.enable_actor_dof_force_sensors(env0, Spherical_Robot0)
...
_root_state = gymtorch.wrap_tensor(_actor_root_state_tensor).view(num_envs, -1, 13)
print(_root_state)
...
actor_root_state = _root_state[:, 0, :]
...
print(Spherical_robot_dof_states)
...
contact_forces = gymtorch.wrap_tensor(net_contact_forces).view(num_envs, -1, 3)
...
roll_integral = torch.zeros(1, 1, device=device)
second_pos_integral = torch.zeros(1, 1, device=device)
...
last_base_euler = torch.zeros_like(actor_root_state[:, 7:10])
yaw_vel = torch.zeros((num_envs,1), device=device)
...
# PID控制初始化
PID_FirstAxis = PIDController(35, 0, 0)
# PID_SecondAxis = PIDController(250, 50, 180)
PID_SecondAxis = PIDController(300, 20, 150) #(110, 45, 110) (200, 20, 120) (1000, 20, 400)
PID_Roll = PIDController(0.9, 0.2, 0)
dt = 0.02
...
while not gym.query_viewer_has_closed(viewer):
    ...
    base_euler_tensor = torch.as_tensor(base_euler, device=device)
    # 得到结果对应 pitch , roll , yaw
    roll = torch.as_tensor(base_euler[:,1])
    yaw = torch.as_tensor(base_euler[:,2])
    yaw_vel = base_euler_tensor[:, 2]
    ...
    if flag == 1:
        ...
        yaw_vel_record.append(yaw_vel[0].item())
    ...
    if count == 500:
        ...
        plot_state2(joint_torque_record, joint_vel_record, joint2_pos_record, quat_record, 
                    ang_vel_all_record, lin_vel_record, pos_y_record, euler_record, ang_vel_record,yaw_vel_record)
        ...
        yaw_vel_record.clear()
        
    #主轴速度控制
    actions_tensor[0,0] =  21.17 * (first_axis_vel - dof_vel[0][0][0]) - 0.97 * (dof_vel[0][0][0] - last_dof_vel[0][0][0]) / sim_params.dt
    # actions_tensor[0,0] =  35 * (first_axis_vel - dof_vel[0][0][0])
    # actions_tensor[0,0] =  PID_FirstAxis.compute(first_axis_vel, dof_vel[0][0][0], dt)
    #副轴位置控制
    # if flag == 0:
    #     actions_tensor[0,1] = 15 * ( second_axis_pos  - dof_pos[0][1][0]) - 5 * dof_vel[0][1][0]
    # roll_integral[0,0] += ( second_axis_pos  - base_euler_tensor[0,1] ) * 0.02
    # second_pos_integral += ( second_axis_pos  - dof_pos[0][1][0]) * 0.02
    # second_pos = 0*roll_integral[0,0] + 0.8* ( second_axis_pos  - base_euler_tensor[0,1]) - 0.35 * (base_euler_tensor[0,1] - last_base_euler[0,1])/0.02
    # second_pos = PID_Roll.compute(second_axis_pos, base_euler_tensor[0,1], dt)
    # second_pos = torch.clip(second_pos, -0.5236, 0.5236)
    actions_tensor[0,1] = 297.46 * ( second_axis_pos  - dof_pos[0][1][0])  - 149.97 * dof_vel[0][1][0]
    # actions_tensor[0,1] = 50 * ( second_axis_pos  - base_euler_tensor[0,1]) - 10 * (base_euler_tensor[0,1] - last_base_euler[0,1])/0.02
    # actions_tensor[0,1] = PID_SecondAxis.compute(second_axis_pos, dof_pos[0][1][0], dt)
    # actions_tensor[0,1] = PID_Roll.compute(second_axis_pos, base_euler_tensor[0,1], dt)
    # actions_tensor[0,0] = 0
    # actions_tensor[0,1] = 0

    torques = torch.clip(actions_tensor,-100.,100.) 
    gym.set_dof_actuation_force_tensor(sim, gymtorch.unwrap_tensor(torques))
    print('---------------')
    print(base_euler_tensor)
    
    
    last_dof_vel[:] = dof_vel[:]
    last_base_lin_vel[:] = base_lin_vel[:]
    last_base_euler[:] = base_euler_tensor[:]
    # update the viewer
