...
def plot_state2(torque_data, vel_data, pos_data, quat_data, ang_vel_data, lin_vel_data, base_pos_data, euler_data, ang_data):
    ...
    # 检查数据长度，确保所有数据长度一致
    data_lengths = [len(torque_data), len(vel_data), len(pos_data), len(quat_data), 
                    len(ang_vel_data), len(lin_vel_data), len(base_pos_data), 
                    len(euler_data), len(ang_data)]
    min_length = min(data_lengths)
    
    # 裁剪所有数据到最小长度
    if len(torque_data) > min_length:
        torque_data = torque_data[:min_length]
    if len(vel_data) > min_length:
        vel_data = vel_data[:min_length]
    if len(pos_data) > min_length:
        pos_data = pos_data[:min_length]
    if len(quat_data) > min_length:
        quat_data = quat_data[:min_length]
    if len(ang_vel_data) > min_length:
        base_ang_vel = base_ang_vel[:min_length]
    if len(lin_vel_data) > min_length:
        lin_vel_data = lin_vel_data[:min_length]
    if len(base_pos_data) > min_length:
        base_pos_data = base_pos_data[:min_length]
    if len(euler_data) > min_length:
        euler_data = euler_data[:min_length]
    if len(ang_data) > min_length:
        ang_data = ang_data[:min_length]
    
    # 基于最小数据长度创建时间轴
    time = np.linspace(0, min_length*sim_params.dt, min_length)
...
props["damping"][0] = 35.0

# 为位置控制设置刚度和阻尼（副轴）
props["stiffness"][1] = 300.0
props["damping"][1] = 150.0
...
if data_record:
    file_path = 'data/torque_outputs2.csv'
...
while not gym.query_viewer_has_closed(viewer):
    ...
    #主轴速度控制 - 直接使用速度目标
    vel_targets = torch.zeros_like(actions_tensor)
    vel_targets[0,0] = first_axis_vel
    
    #副轴位置控制 - 直接使用位置目标
    pos_targets = torch.zeros_like(actions_tensor)
    pos_targets[0,1] = second_axis_pos
    ...
    # 注意：我们不再需要计算和应用力矩
    # 移除了: torques = torch.clip(actions_tensor,-400.,400.)
    # 移除了: gym.set_dof_actuation_force_tensor(sim, gymtorch.unwrap_tensor(torques))
    ...
    if flag == 1:
        ...
        # 如果没有力传感器，可以使用计算的力矩值或者跳过记录
        try:
            joint1_torque_record.append(dof_force_tensor[0, 0].item())
            joint2_torque_record.append(dof_force_tensor[0, 1].item())
            # 注意：只记录一次，不要重复添加
            joint_torque_record.append(dof_force_tensor[0, :].cpu().numpy())
        except:
            # 如果无法访问力传感器数据，则记录零值
            joint1_torque_record.append(0.0)
            joint2_torque_record.append(0.0)
            joint_torque_record.append(np.zeros(2))
        ...
        if data_record:
            Torque_Output_index[0] = round(count * sim_params.dt, 2)
            Torque_Output_index[1] = dof_force_tensor[0, 0]
            Torque_Output_index[2] = dof_force_tensor[0, 1]
            Torque_Output_index[3] = dof_vel[0][0][0]
            Torque_Output_index[4] = dof_vel[0][1][0]
            Torque_Output_index[5] = dof_pos[0][0][0]
            Torque_Output_index[6] = dof_pos[0][1][0]
            Torque_Output_index[7] = base_quat[0][0]
            Torque_Output_index[8] = base_quat[0][1]
            Torque_Output_index[9] = base_quat[0][2]
            Torque_Output_index[10] = base_quat[0][3]
            Torque_Output_index[11] = base_lin_vel[0][0]
            Torque_Output_index[12] = base_lin_vel[0][1]
            Torque_Output_index[13] = base_lin_vel[0][2]
            Torque_Output_index[14] = base_ang_vel[0][0]
            Torque_Output_index[15] = base_ang_vel[0][1]
            Torque_Output_index[16] = base_ang_vel[0][2]
            Torque_Output_index[17] = robot_pos[0][0]
            Torque_Output_index[18] = robot_pos[0][1]
            Torque_Output_index[19] = base_euler_tensor[0][0]
            Torque_Output_index[20] = base_euler_tensor[0][1]
            Torque_Output_index[21] = base_euler_tensor[0][2]
