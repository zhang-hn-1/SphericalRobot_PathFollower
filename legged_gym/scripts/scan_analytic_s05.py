"""Tune high-curvature S paths by fixed length."""
import json,sys
...
def main():
 args=get_args();torch.manual_seed(8501);np.random.seed(8501);configs=[]
 for drive in (.35,.40,.45,.50):
  for stop in (1.2,2.0):
   for kc in (.05,.10):
    for kh in (.5,.8):
     for look in (.2,.4,.6,.8,1.0):configs.append((drive,stop,kc,kh,look))
 assignment=[c for c in configs for _ in range(2)];cfg,_=task_registry.get_cfgs(name='rotunbot_path');cfg.env.num_envs=len(assignment);cfg.noise.add_noise=False;cfg.domain_rand.push_robots=False;cfg.path.curriculum_enabled=False;cfg.seed=8501;args.seed=8501
 env,_=task_registry.make_env(name='rotunbot_path',args=args,env_cfg=cfg);env.path_curriculum_stage=6;env.forced_path_type=3;env.forced_curvature=.5;p=torch.tensor(assignment,dtype=torch.float,device=env.device);drive,stop,kc,kh,look=[p[:,i] for i in range(5)];ids=torch.arange(env.num_envs,device=env.device);batch=torch.arange(env.num_envs,device=env.device);rows=[];lengths=(3.,4.,5.,6.,7.,8.)
 ...
 out.sort(key=lambda r:(r['minimum'],r['overall']),reverse=True);dest=Path('/data/lzq/workspace/SphericalRobot_PathFollower_20260911/artifacts/path_follower/analytic_controller/s05_scan');dest.mkdir(parents=True,exist_ok=True);(dest/'summary.json').write_text(json.dumps(out,indent=2));print(json.dumps(out[:25],indent=2))
