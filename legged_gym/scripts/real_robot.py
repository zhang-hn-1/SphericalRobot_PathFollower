...
def get_action_sequence2(dt, episode_length_buf):
        """
        根据时间计数生成动作序列。
        参数:
...
def Rotunbot_action_list():
    ...
    while not rospy.is_shutdown():
        torque = get_action_sequence2(dt, control_count)
        print(torque)
...
def Rotunbot_RL_Control():
    ...
    model=torch.jit.load("policies/policy_8_24_2.pt")#24_1,2
    print(model)
    ...
    obs_input4 = np.zeros([1, 15], dtype=np.float32)
    ...
    while not rospy.is_shutdown():
        ...
        if control_count == 0:
            initial_yaw = ball_odom[12]
        if control_count%decimation ==0:
            yaw_vel = 25*(ball_odom[12] - last_ball_odom[12])
            yaw = (ball_odom[12] - initial_yaw + math.pi) % (2 * math.pi) - math.pi
            
            obs_input4[0 , :2] = [value * 5.0 for value in command[:]]
            obs_input4[0 , 2:4] = ball_odom[10:12]
            obs_input4[0 , 4] = yaw
            obs_input4[0 , 5:11] = ball_odom[4:10]
            obs_input4[0 , 11] = dof_state[0] * 2.0
            obs_input4[0 , 12] = dof_state[1] * 0.5
            obs_input4[0 , 13:15] = last_action[:]
            # obs_input4[0 , 5:11] = ball_odom[4:10]
            # obs_input4[0 , 11:14] = ball_odom[7:10]
            # obs_input4[0 , 14:17] = ball_odom[7:10]
            obs_input4 = np.clip(obs_input4,-100,100)

            action = model(torch.tensor(obs_input4))[0].detach().numpy()
            action = np.clip(action,-100,100)
            # torque[0] = np.clip(action[0]*1, -8, 8)
            # torque[1] = np.clip(action[1]*0.5, -0.5236, 0.5236)
            torque[0] = np.clip(action[0]*1, -5, 5)
            torque[1] = np.clip(action[1]*1, -0.45, 0.45)
