"""Single Gaussian PPO with intact environment sequences and explicit reset boundaries.

The v2 profile uses explicit parameter groups and a twenty-update body warmup.
...
def source_contract():
    ...
    paths = list((root/'motor_sru').rglob('*.py'))
    paths += [root/name for name in ('dwl/actor_critic_dwl.py', 'scripts/train_motor_sru.py',
        'scripts/evaluate_motor_sru.py', 'envs/base/base_task.py', 'envs/base/legged_robot.py',
...
class MotorPPORunner:
    def __init__(self, env, policy, baseline_metadata, config=None, output=None, device='cpu'):
        ...
        if (policy.hidden_size,policy.num_actor_obs,policy.num_critic_obs,policy.num_actions)!=(512,2941,92,2):
            raise ValueError('This v2 Motor SRU profile fixes H512, actor2941, critic92, actions2')
        self.config = motor_ppo_config() if config is None else copy.deepcopy(config)
        ...
        if self.config['schedule']!='constant' or self.config['body_warmup_updates']<0:
            raise ValueError('v2 requires constant group rates and explicit nonnegative warmup')
        ...
        if not self.baseline_metadata.get('exploration_override'):
            raise ValueError('v2 requires explicit post-baseline exploration initialization provenance')
        self.contract = canonical(dict(schema='motor_sru_single_ppo_v2', environment=env.get_training_contract(),
            baseline=self.baseline_metadata, algorithm=self.config, sources=source_contract(),
            actor_obs=2941, critic_obs=92, critic_remaining_time_column=91, actions=2, recurrent_width=512,
            exploration_override=self.baseline_metadata['exploration_override'],
            memory_update='fresh_depth_only; starts clear before step', distribution='untransformed Gaussian raw joint actions',
    ...
    def _make_optimizer(self):
        groups={name:[] for name in ('actor_body','visual','critic','std')}
        for name,parameter in self.policy.named_parameters():
            if not parameter.requires_grad:
                raise ValueError('Policy gradients must remain enabled; body warmup uses zero LR')
            if name=='std':
                group='std'
            elif name.startswith(('actor.','long_history.')):
                group='actor_body'
            elif name.startswith(('attention.','sru.','visual_projection.')):
                group='visual'
            elif name.startswith(('critic.','critic_extra_projection.')):
                group='critic'
            else:
                raise ValueError('Unassigned PPO parameter: '+name)
            groups[group].append(parameter)
        if any(not value for value in groups.values()) or set(groups)!=set(self.config['learning_rates']):
            raise ValueError('Adam groups must cover body, visual, critic and std exactly')
    ...
    def _validate_obs(self, obs, critic):
        if tuple(obs.shape)!=(self.env.num_envs,2941) or tuple(critic.shape)!=(self.env.num_envs,92):
            raise ValueError('Motor SRU observation contract differs')
    ...
    def collect(self):
        ...
        self.policy.train()
        rows = {key:[] for key in ('obs','critic','starts','mean','std','actions','log_prob','values','rewards','dones')}
        ...
        with torch.no_grad():
            for _ in range(self.config['rollout_steps']):
                ...
                packed = dict(obs=old_obs,critic=old_critic,starts=old_starts,mean=mean,std=distribution.scale,
                    actions=actions,log_prob=distribution.log_prob(actions).sum(-1),values=value.reshape(-1),rewards=reward,dones=done)
                ...
                for index in done.nonzero(as_tuple=True)[0].tolist():
                    ...
                    for key in ('success','collision','out_of_bounds','unstable','timeout','goal_distance','task_index','category_id'):
                        if key in nav:
    ...
    def update(self, rollout):
        ...
        for _ in range(cfg['epochs']):
            ...
            for indices in permutation.chunk(batches):
                ...
                distribution = self.policy.distribution(mean)
                log_prob = distribution.log_prob(rollout['actions'][:,indices]).sum(-1)
                ...
                with torch.no_grad():
                    ...
                    kl = (torch.log(distribution.scale/old_std+1e-5)+(old_std.square()+(old_mean-mean).square())/
                          (2*distribution.scale.square())-.5).sum(-1).mean().item()
        ...
        result=dict(iteration=self.iteration,total_steps=self.total_steps,episodes=n,
            ...
            raw_abs_ge_one_fraction=(rollout['actions'].abs()>=1).float().mean((0,1)).tolist(),
            ...
            minibatches_per_epoch=batches,timeout_bootstrap=False)
        result['by_category']={}
        for identifier,category in enumerate(('required_detour','clear_direct')):
    ...
    def save(self,path):
        ...
        checkpoint=dict(format_version=2,kind='motor_sru_single_ppo',contract=self.contract,
    ...
    def validate_checkpoint(self,checkpoint):
        ...
        if not required.issubset(checkpoint) or checkpoint['format_version']!=2 or checkpoint['kind']!='motor_sru_single_ppo':
            raise ValueError('Not a complete new Motor SRU checkpoint; baseline weights cannot resume training')
        if canonical(checkpoint['contract'])!=self.contract:
            raise ValueError('Motor SRU checkpoint semantic/source/baseline/map contract mismatch')
        ...
        if current.keys()!=incoming.keys() or any(not isinstance(v,torch.Tensor) or v.shape!=current[k].shape or
            ...
            raise ValueError('Invalid Motor SRU parameter keys/shapes/values')
        ...
        for saved_group,live_group in zip(groups,self.optimizer.param_groups):
            ...
            for identifier,parameter in zip(saved_group['params'],live_group['params']):
                ...
                if checkpoint['iteration']>0 and not {'step','exp_avg','exp_avg_sq'}.issubset(state):
