...
def create_sequence_dataset(sim_states, real_states, window_size):
    ...
    for i in range(window_size - 1, len(sim_states)):
        ...
        residual = real_states[i] - sim_states[i]
        residual_targets.append(residual)
    return np.array(sim_seq), np.array(residual_targets)

# 加载仿真与实地状态数据
sim_states = load_json_to_array("output.json", STATE_KEYS)
real_states = load_json_to_array("output_real.json", STATE_KEYS)

# 构建窗口序列数据
X, y = create_sequence_dataset(sim_states, real_states, WINDOW_SIZE)
