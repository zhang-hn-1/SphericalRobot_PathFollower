...
同时 **NeuPAN 的接入接口已做好并端到端验证**，之前 0.7% 的失败率定位为坐标系约定问题。
...
并**定位了 0.7% 的根因**：导出窗口都从原点出发走满 10 m，但坐标轴不是机器人体轴
——首段朝向从 −140° 到 +141°，**144 个里 118 个与机器人朝向差超过 30°**，只做平移会让路径横着铺开。
按首段切线旋转对齐后，同一批路径从 0.7% 变成直线 100%。
